...
class Api:
    ...
    def launch_game(self):
        """마인크래프트 실행 + 플레이 시간 세션 시작.

        필수 모드가 모두 설치돼 있어야 실행된다.
        반환: {"ok":True} 또는 {"ok":False,"error":...,"message":...}
        """
        import time as _time
        ...
        # 이전 세션이 남아 있으면 먼저 정리(누적)한 뒤 새 세션 시작
        self._flush_playtime()
        self._config["session_start"] = int(_time.time())
        preflight.save_config(self._config)
        try:
            preflight.launch_minecraft()
            return {"ok": True}
        except Exception as e:
            return {"ok": False, "error": "launch_failed",
                    "message": f"마인크래프트 실행에 실패했습니다:\n{e}"}

    # ── 설정 화면 동작들 ──────────────────────────────────────────────────
    def open_game_folder(self):
        """마인크래프트(.minecraft) 폴더를 탐색기로 연다."""
        mc = self._minecraft_dir or preflight.find_minecraft_dir(self._config)
        if not mc or not os.path.isdir(mc):
            return {"ok": False, "message": "마인크래프트 폴더를 찾을 수 없습니다."}
        try:
            os.startfile(mc)  # Windows 전용
            return {"ok": True}
        except Exception as e:
    ...
    def update_all(self):
        ...
        tkinter _do_update_all 이식.
        반환: {"ok": True, "success": n, "failed": [{"id","name","reason"}, ...]}
        """
        if not self._registry:
            r = self.ensure_ready()
            if not r.get("ready"):
        ...
        targets = [mid for mid, st in self._mod_statuses.items() if st == "update"]
        failed = []
        for mid in targets:
            res = self.update_mod(mid)
            if not res.get("ok"):
                failed.append({
                    "id": mid,
                    "name": MOD_BY_ID.get(mid, {}).get("name", mid),
                    "reason": res.get("message", "알 수 없음"),
                })

        return {"ok": True, "success": len(targets) - len(failed), "failed": failed}
